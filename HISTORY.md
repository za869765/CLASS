...
## ver5.6（2026-09-16）十月疫苗季關卡新制
- 回溯點：`git tag backup-ver5.4.2-before-oct`（本地＋GitHub）。Apps Script 上原部署為 ver5.5.2，其 10~11 月「週五比照週四」規則由本版取代；ver5.5.2 的審核寫入逐格合併（`writeDraggedPreview` baseRows）已移植保留。
- 僅 2026/10 生效（`OCT_SPECIAL_YMS`），11 月維持現行規則，12 月一切照舊。
- 欄位不擴：D 掛號2／E PRN／F 掛號1／G 前台(週四)／H 查卡／I 場控／J 打針1／K 打針2；週二維持現行；一三五掛號兩線走 J 欄支援池、週四八格全走 K 欄護理師。
- 場控 10/1~10/14 固定陳翊瑄，10/15 起護理師輪流；看板新增「場控」獨立欄。
- 高齡認知 10 月暫停，L 欄只有 10/6 卡介苗。
- 十月次數全進公平帳；每欄月上限改動態。
- 新增「📍 加場人力」按鈕：讀「加場設站」工作表（首次自動建表並填入 7 筆初始資料），可留言異動（員編驗證），純紀錄不計次數。
- 修正：非覆蓋模式 `dow_ex` 作用域錯誤（ReferenceError）。
- 前端：十月表頭以「新名/週二原名」並列、預覽統計／浮標依十月規則分桶、列印同步。
