...
| 前端 | `index.html` |
| 後端 | `Code.gs` |
| 內部版號 | ver5.6（2026-09-16 十月疫苗季關卡新制：2026/10 限定、欄位不擴以日別正名切換、場控前兩週固定陳翊瑄、高齡認知10月暫停、加場設站預覽/留言、審核寫入逐格合併）|
...
## 架構升級摘要
- **ver5.6 十月疫苗季關卡新制（2026/10 限定，`OCT_SPECIAL_YMS` 唯一開關；11 月起自動回現行規則）**：欄位不擴，C~M 沿用，`octColInfo(ci,d)` 為唯一判定來源（名稱／池／是否排班）。一三四五：D 掛號2、E PRN、F 掛號1、G 前台(僅週四)、H 查卡、I 場控、J 打針1、K 打針2；一三五掛號1/2 由 J 欄支援池指針各出 1 人，週四 D~K 八格全走 K 欄護理師；週二維持現行欄位；L 欄只剩首週二卡介苗（`isCognitiveActive` 十月回 false）。場控 1~14 日固定 `OCT_CTRL_FIXED`（陳翊瑄），15 日起 K 欄輪流；看板 `wdCounts` 第 14 欄「場控」獨立計數。十月一三五計數併入「週四桶」，每欄月上限改動態（總格數÷池人數進位+1）。加場設站（`加場設站` 工作表，`getExtraSessions`/`addExtraSessionComment`）純紀錄不計次數。回溯點：git tag `backup-ver5.4.2-before-oct`。
- **ver5.3 卡介苗輪序改接續制**：pre-pass 改查「上月卡介苗日（首個工作週二）原始站班者」（`buildOriginalScheduleMap` 反推，換班/拖曳不進輪序帳，與值班/停班2線跨月接續同基準），Q 欄順位 +1 為本月人選；查無起點（上月無表/空白/不在 Q 欄）才回退固定日曆公式（ver5.3.1 起 BASE 重錨 2026/11=Q1）；另 `_BCG_ANCHOR` 一次性錨定 2026/10=陳伶雯（7~9月偏離輪序後當事人自行換班協調，10月起重啟：10陳伶雯→11蔡錦慧→12呂佳卉）
